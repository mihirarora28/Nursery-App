...
class OrdersProvider with ChangeNotifier{
  ...
  Future<void> addItem(String id, double totalAmount, List<Cart> products) async{
    ...
    try {
      final response = await https.post(Uri.parse(url), body: json.encode({
      ...
      _items.add(OrderItem(dateTime: DateTime.now(),
          id: id,
          products: products,
          totalAmount: totalAmount));
      notifyListeners();
   ...
   final List<OrderItem> newOrder =[];
    data.forEach((key, value) {
      newOrder.add(OrderItem(dateTime: DateTime.parse(value['dateTime']) , id: key, products: (value['products'] as List<dynamic>).map((ee) => Cart(
        ...
        title: ee['title']
      )).toList(), totalAmount: value['totalAmount']));
    });
